...
|---|---|---|---|---|
| OPT-01 | P0 | 最新思考窗口 + 完整思考查询入口 | 中 | 已实施（见任务文档实施记录） |
| OPT-02 | P1 | 快照合并、终态优先、分类重试、双时间戳 | 中 | 已实施（见任务文档实施记录） |
| OPT-03 | P1 | 序列化预算、分页/分段、完整结果交付 | 中 | 已实施（附件路径除外，见任务文档） |
| OPT-04 | P1 | 入站持久化、幂等、恢复状态 | 大 | 已实施（真实 kill 注入与 comment 入口除外，见任务文档） |
| OPT-05 | P2 | 能力声明、等待输入/审批闭环 | 大 | 能力矩阵已实施；纵向闭环未实施（见任务文档） |
| OPT-06 | P2 | 任务视图、完成凭据、信息密度 | 中 | 完成凭据分片已实施；视图重设计未实施（见任务文档） |
