...
export class CodeBuddyAdapter implements AgentAdapter {
  ...
  constructor(opts: CodeBuddyAdapterOptions = {}) {
    this.binary = opts.binary ?? process.env.LARK_CHANNEL_CODEBUDDY_BIN ?? 'codebuddy';
  ...
  run(opts: AgentRunOptions): AgentRun {
    ...
    // Flush a trailing stderr line that arrived without a newline so it still
    // reaches the log (review FB-003).
    child.stderr.on('close', () => {
      if (stderrBuffer.trim()) {
        log.warn('agent', 'stderr', { line: stderrBuffer });
        stderrBuffer = '';
      }
    });
